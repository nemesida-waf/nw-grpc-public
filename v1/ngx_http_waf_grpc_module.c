...
#include "ngx_http_waf_grpc_module.h"
#include "request_parser.h"
#include "event_description.h"

#include "nwaf_grpc_parse.h"
#include "nwaf_grpc_process.h"
...
static ngx_int_t ngx_http_nwaf_grpc_body_output_filter(void *data, ngx_chain_t *in);
...
static ngx_int_t ngx_http_nwaf_grpc_filter(void *data, ssize_t bytes);
...
static char *ngx_http_nwaf_grpc_pass(ngx_conf_t *cf, ngx_command_t *cmd,
    void *conf);
...
ngx_module_t  ngx_http_waf_grpc_module;
extern ngx_module_t  ngx_http_grpc_module;

static ngx_command_t  ngx_http_nwaf_grpc_commands[] = {

    { ngx_string("nwaf_grpc_pass"),
      NGX_HTTP_LOC_CONF|NGX_HTTP_LIF_CONF|NGX_CONF_TAKE1,
      ngx_http_nwaf_grpc_pass,
...
static ngx_http_module_t  ngx_http_nwaf_grpc_module_ctx = {
...
ngx_module_t  ngx_http_waf_grpc_module = {
    NGX_MODULE_V1,
    &ngx_http_nwaf_grpc_module_ctx,             /* module context */
    ngx_http_nwaf_grpc_commands,                /* module directives */
...
ngx_int_t ngx_http_grpc_waf_init(ngx_conf_t *cf) {
  ...
  wmc = ngx_http_conf_get_module_main_conf(cf, ngx_http_waf_module);
  wmc->grpc_module = 1;
...
static ngx_int_t
ngx_http_nwaf_grpc_handler(ngx_http_request_t *r)
{
    ngx_int_t                  rc;
    ...
    ngx_http_nwaf_grpc_ctx_t  *nwaf_grpc_ctx;
    ...
    rctx = ngx_http_get_module_ctx(r, ngx_http_waf_module);
    if (rctx != NULL) {
      if (rctx->delayed || rctx->clam_delayed || rctx->icap_delayed) {
        return NGX_AGAIN;
      }
    }

    nwaf_grpc_ctx = ngx_http_get_module_ctx(r, ngx_http_waf_grpc_module);
    if (nwaf_grpc_ctx == NULL) {
      nwaf_grpc_ctx = ngx_pcalloc(r->pool, sizeof(ngx_http_nwaf_grpc_ctx_t));
      if (nwaf_grpc_ctx == NULL) {
        return NGX_HTTP_INTERNAL_SERVER_ERROR;
      }

      nwaf_grpc_ctx->header_sent = 0;
      nwaf_grpc_ctx->in = NULL;
      nwaf_grpc_ctx->request = r;
      ngx_http_set_ctx(r, nwaf_grpc_ctx, ngx_http_waf_grpc_module);
    }
    ...
    u->input_filter = ngx_http_nwaf_grpc_filter;
...
ngx_http_grpc_create_request(ngx_http_request_t *r)
{
    ...
    u->output.output_filter = ngx_http_nwaf_grpc_body_output_filter;
...
static ngx_int_t
ngx_http_nwaf_grpc_body_output_filter(void *data, ngx_chain_t *in)
{
    ngx_http_request_t  *r = data;
    ngx_http_grpc_ctx_t    *ctx;
    ngx_int_t              nwaf_result;
    ngx_http_request_ctx_t    *rctx;
    ngx_http_nwaf_grpc_ctx_t       *nwaf_grpc_ctx;
    ngx_chain_t *n_in;
    ...
    if (in) {
      if ((ngx_buf_size(in->buf) != 0) || ngx_buf_special(in->buf)){
        if (ngx_chain_add_copy(r->pool, &nwaf_grpc_ctx->in, in) != NGX_OK) {
          return NGX_ERROR;
        }
      }
    }

    if (!nwaf_grpc_ctx->header_sent) {
      nwaf_grpc_ctx->header_sent = 1;
      n_in = n_in->next;
    }
    ...
    while (in && limit > 0) {
        ...
        do {
            ...
            *ll = cl;
            ...
            b->tag = (ngx_buf_tag_t) &ngx_http_nwaf_grpc_body_output_filter;
//            b->tag = (ngx_buf_tag_t) &ngx_http_grpc_body_output_filter;
    ...
    *ll = NULL;
    ...
    ngx_chain_update_chains(r->pool, &ctx->free, &ctx->busy, &out,
                           (ngx_buf_tag_t) &ngx_http_nwaf_grpc_body_output_filter);
//    ngx_chain_update_chains(r->pool, &ctx->free, &ctx->busy, &out,
...
static ngx_int_t
ngx_http_nwaf_grpc_filter(void *data, ssize_t bytes)
{
    ngx_http_grpc_ctx_t  *ctx = data;
...
ngx_http_grpc_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    {
        clcf->handler = ngx_http_nwaf_grpc_handler;
    }
...
static char *
ngx_http_nwaf_grpc_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
